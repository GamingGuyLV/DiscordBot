...
class HelpSelectView(View):
    ...
    async def callback(self, select, interaction):
        if select.values[0] == "General":
            ...
            embed.add_field(name="***Moderation***", value="`Clear`  `Kick`  `Ban`  `Unban`  `Banlist`", inline=False)
            embed.add_field(name="***Music***", value="`Connect`  `Disconnect`  `<Play>`  `<Pause>`  `<Resume>`  `<Skip>`  `<LoopSong>`  `<LoopQueue>`  `<Current>`", inline=False)
        ...
        if select.values[0] == "Music":
            ...
            embed.set_author(name="These are music commands and their usage.")
            embed.add_field(name="***Formatting***", value="Commands in <> are work in progress.", inline=False)
            embed.add_field(name="***Connect***", value="`Connects the bot to your voice channel. Usage: /connect`", inline=False)
            embed.add_field(name="***Disconnect***", value="`Disconnects the bot from your voice channel. Usage: /disconnect`", inline=False)
            embed.add_field(name="***<Play>***", value="`Plays a youtube song from youtube URL. Usage: /play $URL$`", inline=False)
            embed.add_field(name="***<Pause>***", value="`Pauses the playback. Usage: /pause`", inline=False)
            embed.add_field(name="***<Resume>***", value="`Resumes the playback. Usage: /resume`", inline=False)
            embed.add_field(name="***<Skip>***", value="`Skips the current playback file. Usage: /skip`", inline=False)
            embed.add_field(name="***<LoopSong>***", value="`Loops the current playback file. Usage: /loopsong`", inline=False)
            embed.add_field(name="***<LoopQueue>***", value="`Loops the entire playback queue. Usage: /loopqueue`", inline=False)
            embed.add_field(name="***<Current>***", value="`Prints out the current playback file. Usage: /current`", inline=False)
            embed.set_footer(text=f"Code version - {code_version}")
            await interaction.response.edit_message(embed=embed)
